...
import { createOrder, getProduct, sendMessage, updateProduct } from '../api';
      ...
      .addEventListener('click', async () => {
        ...
        if (data.error) {
          ...
          for (let i = 0; i < cartItems.length; i++) {
            const product= getProduct(cartItems[i].product);
            console.log(cartItems[i].product);
            const data= await updateProduct({
              id: cartItems[i].product,
              name: cartItems[i].name,
              info: cartItems[i].info,
              price: cartItems[i].price,
              cantidad: cartItems[i].cantidad - orderItems[i].cantidad1,
          ...
          /*
          for (let i = 0; i < orderItems.length; i++) {
              const product=getProduct(orderItems[i].id);
              const data= await updateProduct({
                id: product.id,
                name: product.name,
                info: product.info,
                price: product.price,
                cantidad: product.cantidad - orderItems[i].cantidad1,
                

            });   

            console.log(product.cantidad - orderItems[i].cantidad1) 
          };
          */
          if(sendMessage(msg)){
              ...
              ${orderItems
                ...
                <li>
                  ...
                  <div class="cart-name">
                    ...
                    <div> Cantidad: ${item.cantidad1} </div>
                  </div>
